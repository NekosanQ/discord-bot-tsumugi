...
void test('API entrypointはimportだけで環境読込・signal登録・接続を行わない', async (): Promise<void> => {
    const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'tsumugi-api-import-'));

    try {
        const childEnvironment = { ...process.env };
        childEnvironment.API_SERVICE_TOKEN = '';
